...
class InternalDecisionNode(object):
    ...
    def predict(self, x_TF):
        ...
        # Call predict on the left child
        left_predictions = self.left_child.predict(x_TF[left_mask])

        # Call predict on the right child
        right_predictions = self.right_child.predict(x_TF[right_mask])
        
        # Aggregate all predictions and return one array
        yhat_T = np.empty(x_TF.shape[0], dtype=np.float64)
        yhat_T[left_mask] = left_predictions
        yhat_T[right_mask] = right_predictions
