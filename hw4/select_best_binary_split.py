...
def select_best_binary_split(x_NF, y_N, MIN_SAMPLES_LEAF=1):
    ...
    for f in range(F):
        ...
        for v, thresh in enumerate(possib_xthresh_V):
            ...
            # Check if we have enough samples in each child
            if len(y_L) < MIN_SAMPLES_LEAF or len(y_R) < MIN_SAMPLES_LEAF:
            ...
            left_cost_V[v] = np.sum((y_L - left_yhat_V[v]) ** 2)
            right_cost_V[v] = np.sum((y_R - right_yhat_V[v]) ** 2)
        ...
        # Pick out the split candidate that has the best cost
