...
def calc_mean_BCE_from_probas(ytrue_N, yproba1_N):
    ...
    # Handle empty input lists properly
    if N == 0:
        return 0.0
    ...
    mean_binary_cross_entropy = np.mean(-ytrue_N * np.log2(yproba1_N) - (1 - ytrue_N) * np.log2(1 - yproba1_N))
...
def calc_mean_BCE_from_scores(ytrue_N, scores_N):
    ...
    scores_and_zeros_N2 = np.stack((np.zeros(N), flipped_scores_N), axis=1)
    ...
    return bce_score  # Return the computed binary cross entropy
